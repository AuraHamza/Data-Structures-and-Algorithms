...
class WMS{
    ...
    // left subtree agar value choti ho sakti hai
    if (low < head->id)
        Kprint(head->left, low, high);

    // print only when in range
    if (low <= head->id && head->id <= high)
        cout << "ID: " << head->id << " || Description: " << head->desp << endl;

    // right subtree agar value badi ho sakti hai
    if (high > head->id)
        Kprint(head->right, low, high);
}
Node* deleteMultiples(Node* head, int n) {
    if (head == NULL)
        return NULL;

    // Pehle left aur right ko process karo
    head->left = deleteMultiples(head->left, n);
    head->right = deleteMultiples(head->right, n);

    // Ab current node ko check karo
    if (head->id % n != 0) {
        // Ye multiple nahi hai -> delete
        head = Delete(head, head->id);
    }
